...
async page => {
  ...
  try {
    ...
    check(await mobile.locator('link[rel="apple-touch-icon"]').getAttribute('href') === 'https://homepage-assets.mathtranslations.org/site-icons/apple-touch-icon-7ef542db.png', 'Apple icon missing');
  } finally {
    await context.close();
  }
