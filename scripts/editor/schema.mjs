...
export const documents = {
  ...
  literature: { file: 'literature.json', label: '文学', schema: object('文学', {
    books: array('书籍', { title, englishTitle: text('英文名'), originalTitle: text('原名'), author: text('作者'), originalAuthor: text('作者原名'),
      translator: text('译者'), edition: text('出版社与版本'), year, originalYear: number('作品首次出版年份（用于排序）', { min: 1 }), coverUrl: image('封面'), collection: text('所属系列'), collectionEnglishTitle: text('系列英文名'), partOrder: number('系列顺序', { min: 1 }), partLabel: text('分卷标记'), firstRead: text('初读日期'), reread: text('重读日期'), review, hidden }),
